...
class PatchEmbedDust3R(PatchEmbed):
    def forward(self, x, prompt_embeddings, **kw):
        B, C, H, W = x.shape
        ...
        x = self.proj(x)  # torch.Size([2, 1024, 10, 32])
...
class ManyAR_PatchEmbed (PatchEmbed):
    ...
    def forward(self, img, prompt_embeddings, true_shape):
        B, C, H, W = img.shape
        ...
        x = self.norm(x + prompt_embeddings)
        return x, pos
